...
import katana.procedures.FRS_tick_prProcedure;
...
public class FurnaceRecipeViewerMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public FurnaceRecipeViewerMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 600 + 8 + sj * 18, -33 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 600 + 8 + si * 18, -33 + 142));
	}
	...
	public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.END && entity.containerMenu instanceof FurnaceRecipeViewerMenu) {
			Level world = entity.level;
			double x = entity.getX();
			double y = entity.getY();
			double z = entity.getZ();
			FRS_tick_prProcedure.execute(world, entity);
		}
	}
}
